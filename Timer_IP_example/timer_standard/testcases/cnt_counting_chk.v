...
task run_test();
    ...
    begin
        ...
        $display("check timer_en = 0, timer does not count");
        //read current value
        test_bench.apb_rd(ADDR_TDR0, task_rdata);
        cnt[31:0] = task_rdata;
        test_bench.apb_rd(ADDR_TDR1, task_rdata);
        ...
        if( cnt1 === cnt) begin
			$display("--------------------------------------------------------");
			$display("t=%10d PASSED: cnt does not increase when timer_en is 0 ",$time);
			$display("--------------------------------------------------------");
        end else begin
			$display("------------------------------------------------");
			$display("t=%10d FAIL: cnt increased when timer_en is 0 ",$time);
